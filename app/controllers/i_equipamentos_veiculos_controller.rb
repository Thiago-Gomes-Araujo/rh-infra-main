class IEquipamentosVeiculosController < ApplicationController
  ...
  def create
    @i_equipamento = IEquipamentoVeiculo.new(i_equipamento_params.merge(i_ambiente_predial: @i_ambiente_predial))
  ...
  def set_i_equipamento
    @i_equipamento = IEquipamentoVeiculo.find(params[:id])
  end
  ...
  def i_equipamento_params
    params.require(:i_equipamento_veiculo).permit(IEquipamentoVeiculo.column_names.reject { |col| ["deleted_at", "created_by", "updated_by"].include?(col) }.map(&:to_sym))
  end
end
